...
void initQueue(struct Queue *q) {
    ...
    pthread_cond_init(&q->th_cond_hasitem, NULL);
}
...
void freeQueue(struct Queue *queue) {
    ...
    pthread_mutex_destroy(&queue->th_mutex_queue);  // release the mutex
    pthread_cond_destroy(&queue->th_cond_hasitem);   // release the condition
    free(queue);
}
...
void enqueue(struct Queue *q, struct BurstItem *i) {
    ...
    pthread_cond_signal(&q->th_cond_hasitem);
...
struct BurstItem *dequeue(struct Queue *q) {
    ...
    while (q->front == NULL) {
        pthread_cond_wait(&q->th_cond_hasitem, &q->th_mutex_queue);
    }
    ...
    while (1) {
        ...
        if (strcmp(alg, "RR") == 0 && burstItem->remainingTime > timeQuantum) {
            ...
            if (m == 3 && burstItem->remainingTime > 0) {
                time = getTime();
                my_print(
                        "time slice is expired at time=%ld, in cpu=%d, for burst pid=%d, burstlen=%d, remainingtime=%d\n",
                        time, burstItem->tid, burstItem->pid,
                        burstItem->burstLength, burstItem->remainingTime);
            }
            continue;
